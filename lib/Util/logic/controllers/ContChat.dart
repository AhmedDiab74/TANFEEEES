...
class ContChat extends GetxController {
  ...
  RxString swowondertimelineflutterappersGif = ''.obs;
...
//رؤوية الرسالة
  ...
  void get() async {
    ...
    //           });
    // }
    final ameen = GetUserMessgesApi.chat(Get.arguments['usd'].toString(), '')
        .then((value) {
  ...
  sendMessgaSoketMedia(userid, sours) async {
    var senmess = await ApiSendRecordAudio.send(
        use: userid, path: ImagePost!.path, filename: filename!, sours: sours);
    print(senmess);
