...
class _ChatPageState extends State<ChatPage> {
  static final LottieBuilder _generatingLottie = Lottie.asset(images.loading2);
  ...
  Future<void> _speak(String text) async {
    // If it is playing, stop it first
        ...
        child: Column(
          children: [
            ...
            Image(
              width: 18,
              height: 18,
              image: AssetImage(images.tip_icon),
            ),
  ...
  Widget _renderMessageItem(Map message, int index) {
    String role = message['role'];
    String defaultAvatar = images.logo;
    ...
    if (role == 'user') {
      defaultAvatar = getimydata.data[0].avatar;
      defaultRoleName = getimydata.data[0].name;
                ...
                child: Row(
                  children: [
                    ClipRRect(
                      ...
                      clipBehavior: Clip.antiAlias,
                      child: defaultAvatar.startsWith('http')
                          ? CachedNetworkImage(
  ...
  Widget _renderShareWidget(Map message) {
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Image(
          image: AssetImage(images.share_message_icon),
          width: 22,
        ),
      ),
    );
  ...
  Widget _renderVoiceWidget(Map message) {
    return GestureDetector(
      ...
      child: Container(
        padding: const EdgeInsets.fromLTRB(2, 2, 2, 2),
        child: Image(
          image: AssetImage(images.voice_icon),
          width: 26,
        ),
      ),
    );
  ...
  Widget _renderCopyWidget(Map message) {
    return GestureDetector(
      ...
      child: Container(
        padding: const EdgeInsets.fromLTRB(2, 2, 2, 2),
        child: Image(
          image: AssetImage(images.chat_copy_icon),
          width: 26,
        ),
      ),
    );
  ...
  Widget _renderRegenerateWidget(int index) {
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Image(
          image: AssetImage(images.refresh_icon),
          width: 26,
        ),
      ),
    );
